...
#include "applicationFunctions.h" //Send and read application msgs between processes.
#include "routingTable.h" //Routing utility.
#include "routing_daemon.h" //Signatures of this source file.
...
u_int8_t REQUEST[3] = REQ; //Request.
u_int8_t RESPONSE[3] = RSP; //Response.
u_int8_t UPDATE[3] = UPD; //Update.
u_int8_t HELLO[3] = HEL; //Hello.
u_int8_t ALIVE[3] = ALV; //Keep-alive.
...
/*
    ...
    @Param  The destination mip, 0xFF is not broadcast but unicast to all active neighbours.
*/
void sendUpdate(u_int8_t dst_mip)
{
  if(debug)
  {
    ...
    {
      timestamp();
      printf("SENDING UPDATE-MSG TO ALL ACTIVE NEIGHBOURS\n");
    }

    else
    {
      timestamp();
      printf("SENDING UPDATE-MSG TO %u\n", dst_mip);
    }
  }
  ...
  if(dst_mip != 0xFF)
  {
    ...
    char* buffer = calloc(1, sizeof(UPDATE) + (sizeof(u_int8_t))); //size of UPDATE + amount (u_int8_t).
    ...
    for(int i = 0; i < 255; i++)
    {
      ...
      if(findEntry(i) != NULL)
      {
        //If the route was recived from this mip dont send it back, unless its infinite.
        if(findEntry(i) -> next_hop != dst_mip || findEntry(i) -> cost == 255)
        {
    ...
    if(updateStructure.amount > 0)
    {
      if(debug)
      {
        timestamp();
        printf("SENT UPDATE TO NEIGHBOUR WITH MIP: %u\n", dst_mip);
      }

      sendApplicationMsg(routingSocket, dst_mip, buffer, 1, 4 + (updateStructure.amount * sizeof(routingEntry))); //size of fields in updateStructure + length of actual update data.
    }
    //Free the content.
  ...
  else if(dst_mip == 0xFF)
  {
    ...
    while(tempNode != NULL)
    {
       ...
       for(int i = 0; i < 255; i++)
       {
         ...
         {
           //If the route was recived from this mip dont send it back, unless its infinite.
           if(findEntry(i) -> next_hop != current -> mip || findEntry(i) -> cost == 255)
           {
             //If there is a entry, allocate space and copy over the data. Increase counter.
             updateStructure.amount = updateStructure.amount + 1;
             updateStructure.data = realloc(updateStructure.data, sizeof(routingEntry) * updateStructure.amount);
       ...
       if(updateStructure.amount > 0)
        sendApplicationMsg(routingSocket, current -> mip, buffer, 1, 4 + (updateStructure.amount * sizeof(routingEntry))); //size of fields in updateStructure + length of actual update data.
       //Free the content.
       free(updateStructure.data);
       free(buffer);
     }
...
void handleIncomingMsg()
{
  ...
  //Inspect first 3 bytes of data to determine what kind of msg arrived.
  ...
  else if(memcmp(UPDATE, applicationMsg -> payload, sizeof(UPDATE)) == 0)
  {
    ...
    while(counter < updateStructure.amount)
    {
      ...
      //increase counter to be equal to start of next entry.
      currentPos = currentPos + sizeof(routingEntry);

      //if there is no entry in the routingtable, and the entry from the UPDATE does not have infinite cost.
      if(findEntry(entry.mip_address) == NULL && entry.cost != 255)
      {
        addToRoutingTable(entry.mip_address, entry.cost + 1, applicationMsg -> address);
        changed = true;
      }

      //if there is a entry at the same index as the entry from the UPDATE.
      else if(findEntry(entry.mip_address) != NULL)
      {
        //if the entry has cost of infinite and the route this
        //node knows about about goes trough the sender, set cost to infinite.
        if(entry.cost == 255 && findEntry(entry.mip_address) -> next_hop == applicationMsg -> address)
        {
    ...
    if(changed)
    {
      ...
      //If table has changed, update all active neighbours.
      sendUpdate(0xFF);
    }
  ...
  else if(memcmp(HELLO, applicationMsg -> payload, sizeof(HELLO)) == 0)
  {
    ...
    if(!findNeighbour(applicationMsg -> address))
    {
      if(debug)
      {
        timestamp();
    ...
    else if(findNeighbour(applicationMsg -> address))
    {
      timestamp();
      printf("%u SEEMS TO HAVE DISCONNECTED FOR A WHILE -- BACK IN ROUTING-NETWORK NOW\n\n", applicationMsg -> address);

      //Update their time, this is a Implicitly a keep-alive.
      updateTime(applicationMsg -> address);
      //Routingtable has not changed, so only unicast msg to this neighbour only.
      sendUpdate(applicationMsg -> address);
    }
